...
#https://blog.csdn.net/qq_38828370/article/details/119696756
...
def smooth(image, sigma = 1.4, length = 5):
    ...
    Returns:
        the smoothed image
    """
    # 1、生成高斯滤波核 Compute gaussian filter
    k = length // 2
    ...
    # 2、归一化 Batch Normalization
    gaussian = gaussian / np.sum(gaussian)

    # 3、高斯滤波 Use Gaussian Filter
    W, H = image.shape
...
def canny(image, threshold1, threshold2, apertureSize):
    
    image = smooth(image,sigma=1,length=7)
    gradients, direction = get_gradient_and_direction(image)
    nms = NMS(gradients=gradients, direction=direction)
    output_image = double_threshold(nms, 30, 90)

    out1 = cv2.Canny(image, 50, 90, apertureSize=3)
    cv2.imshow("man",output_image)
    cv2.imshow("opencv",out1)
    cv2.waitKey()
    m1 = np.max(nms)
    m2 = np.min(nms)
    return nms
...
if __name__ == "__main__":
    img = cv2.imread("D:/desktop/2.png",cv2.IMREAD_GRAYSCALE)
    img = canny(image=img, threshold1=30, threshold2=60, apertureSize=5)

    dx = cv2.Sobel(src, cv2.CV_16S,1,0,ksize=3)
    dy = cv2.Sobel(src, cv2.CV_16S,0,1,ksize=3)
    _norm = np.zeros_like(src)
    for i in range(src.shape[0]):
        r,c = i//src.shape[1],i%src.shape[1]
        _dx = dx[r,c]
        _dy = dy[r,c]
        _norm[r,c] = _dx*_dx + _dy*_dy
        cv2.Canny()
