...
lr = 1e-3
resume_epoch = 0
pre_weights = os.path.join(config.checkpoints_dir, 'best.dict')
...
def save_checkpoint(state, epoch, loss_cls, loss_regr, loss, ext='dict'):
...
if __name__ == '__main__':
    ...
    for epoch in range(resume_epoch+1, epochs):
        ...
        if best_loss_cls > epoch_loss_cls or best_loss_regr > epoch_loss_regr or best_loss > epoch_loss:
            ...
            dict = {"model_state_dict": best_model.state_dict(),
                    "loss_reg": best_loss_regr,
                    "loss_cls": best_loss_cls}
            
            torch.save(dict, "./checkpoints/best.dict")
