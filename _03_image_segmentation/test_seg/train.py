...
from model import DeepLabV3_2, UNet,deeplabv3
import datetime
import tqdm
...
def train(opt):
    ...
    net = deeplabv3()  # deeplabv3() UNet() DeepLabV3_2()
    net.to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrain', default='./run/train/best_kongdong_power.pth', help='指定权重文件，未指定则使用官方权重！')  # 修改
    parser.add_argument('--out_path', default='./run/train', type=str)  # 修改
    parser.add_argument('--weights', default='best_kongdong_power.pth', help='指定权重文件，未指定则使用官方权重！')
